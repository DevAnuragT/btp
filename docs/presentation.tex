...
\institute[BTP 2026]{Department of Computer Science \& Engineering\\Bachelor of Technology Project (BTP)}
\date{August 2026}
...
% ==========================================
\begin{frame}{Problem Statement: Next-Day 24-Hour Stock Status Prediction}
  \begin{columns}
    \begin{column}{0.55\textwidth}
      \begin{block}{The Operational Challenge in Fresh Retail}
        \begin{itemize}
          \item Perishable fresh goods suffer from \textbf{rapid intra-day demand spikes} and supply replenishment delays.
          \item Stockouts lead to direct financial loss, customer churn, and store operational inefficiency.
        \end{itemize}
      \end{block}
      ...
      \begin{block}{Predictive Target Formulation}
        \begin{itemize}
          ...
          \item $y_h = 1$ indicates SKU stockout during operational hour $h$.
        \end{itemize}
      \end{block}
    \end{column}
    ...
    \begin{column}{0.42\textwidth}
      \centering
      \includegraphics[width=\textwidth]{images/pipeline_overview.png}\\
      \vspace{2pt}
      \tiny{Figure 1: End-to-End Hourly Stock Status Forecasting Framework}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Literature Review \& Related Work}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{block}{Limitations of Traditional Daily Aggregation}
        \begin{itemize}
          \item Traditional retail models forecast 1 scalar daily demand value.
          \item \textbf{Key Flaw}: Hides intraday stockout timing (e.g. stockout at 11 AM vs 8 PM requires vastly different store actions).
        \end{itemize}
      \end{block}
      
      \begin{block}{Deep Time-Series Transformers}
        \begin{itemize}
          \item \textbf{PatchTST} (Zeng et al., 2023) \& \textbf{TFT} (Lim et al., 2021).
          \item Attention maps point-wise tokens $\rightarrow$ prone to \textbf{overparameterization} (280k+ params) on short sequence splits.
        \end{itemize}
      \end{block}
    \end{column}
    
    \begin{column}{0.5\textwidth}
      \begin{block}{Linear Decomposition Models}
        \begin{itemize}
          \item \textbf{DLinear} (Zeng et al., 2023): Decomposes sequences into trend (moving-average) and seasonal components.
          \item Extremely parameter-efficient ($<15\text{k}$ parameters) with strong generalization.
        \end{itemize}
      \end{block}
      
      \begin{block}{Explainable AI in Time Series}
        \begin{itemize}
          \item \textbf{Integrated Gradients} (Sundararajan et al., 2017) \& \textbf{SHAP}.
          \item Used to inspect feature attributions and guide architectural decoupling.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Data Selection Strategy: FreshRetailNet-50K \& SKU Scoring}
  \begin{block}{Unit of Analysis}
    Time series defined at the individual SKU-store grain: 
  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      \begin{block}{Why Statistically Filter SKUs?}
        \begin{itemize}
          \item Fresh retail datasets contain thousands of dormant SKUs.
          \item We focus evaluation on \textbf{high-impact, stockout-prone SKUs} driving 80\%+ of store revenue loss.
        \end{itemize}
      \end{block}
    \end{column}
    ...
    \begin{column}{0.42\textwidth}
      \begin{block}{SKU Statistical Ranking Formula}
        $$\begin{aligned}
          \text{Score} &= 0.40 \cdot \text{Sales} \\
          &+ 0.25 \cdot \text{StockoutHours} \\
          &+ 0.20 \cdot \text{StockoutDays} \\
          &+ 0.15 \cdot \text{SalesStd}
        \end{aligned}$$
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{ML Explainability: SHAP \& Integrated Gradients Analysis}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{block}{Integrated Gradients (IG) Feature Attribution}
        \begin{itemize}
    ...
    \begin{column}{0.5\textwidth}
      \begin{alertblock}{Identification of Feature Cross-Talk}
        \begin{itemize}
          \item Feeding demand \& inventory features into a single shared LSTM recurrent state causes \textbf{gradient noise and interference}.
          \item \textbf{Architectural Takeaway}: Inspired the design of \textbf{Dual-Stream Decoupling} to isolate sales and inventory signals into parallel streams.
        \end{itemize}
      \end{alertblock}
    \end{column}
  \end{columns}
...
\begin{frame}{Architectural Innovation 1: Dual-Stream Feature Decoupling}
  \begin{columns}
    ...
    \begin{column}{0.42\textwidth}
      \begin{block}{Dual-Stream Mechanism}
        \begin{itemize}
          \item Runs \textbf{two parallel 16-dim BiLSTMs}:
            \begin{enumerate}
...
% ==========================================
% SLIDE 11: MASTER EMPIRICAL VALIDATION LEADERBOARD
% ==========================================
